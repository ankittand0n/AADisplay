...
object ProbeLogger {
    ...
    fun logClassLoad(className: String, dexPath: String? = null, success: Boolean, error: Throwable? = null) {
        ...
        if (!success || isProbeMode) {
            ...
            if (error != null) sb.append("Error: ${error.javaClass.simpleName} - ${error.message}\n")
            sb.append("=================================")
            log(TAG, sb.toString())
        }
    }
    ...
    fun logMethodLookup(
        ...
        if (!success || isProbeMode) {
            ...
            if (paramTypes != null) sb.append("Params: ${paramTypes.joinToString { it.simpleName }}\n")
            sb.append("Android: ${Build.VERSION.RELEASE} (SDK ${Build.VERSION.SDK_INT})\n")
            if (error != null) sb.append("Error: ${error.javaClass.simpleName} - ${error.message}\n")
            sb.append("=================================")
            log(TAG, sb.toString())
        ...
        if (!success || isProbeMode) {
            ...
            if (error != null) {
                ...
                if (isProbeMode) {
                    ...
                    error.stackTrace.take(10).forEach { sb.append(" at $it\n") }
                }
            }
            sb.append("=================================")
            log(TAG, sb.toString())
    ...
    fun getHistorySummary(): String {
        ...
        synchronized(probeHistory) {
            ...
            ProbeType.entries.forEach { type ->
