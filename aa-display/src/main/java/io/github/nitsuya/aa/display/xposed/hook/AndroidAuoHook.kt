...
object AndroidAuoHook : BaseHook() {
    ...
    @Volatile private var isAadInitFailed = false
    override fun init(lpparam: XC_LoadPackage.LoadPackageParam) {
        try {
            val processName = lpparam.processName
            val hooks = listOf(AaBasicsHook, AaSignatureHook, AaDpiHook, AaBtnEventHook, AaUiHook, AaPropsHook)
            ...
            if (!configPreferences.file.canRead()) {
                log(tagName, "load configPreferences fail - continuing with defaults")
            }
